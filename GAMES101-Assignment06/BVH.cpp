...
Intersection BVHAccel::getIntersection(BVHBuildNode* node, const Ray& ray) const
{
    ...
    if (node->left == nullptr && node->right == nullptr) {
        return node->object->getIntersection(ray);//与三角形面求交，返回交点属性
    }
    
    Intersection hit1 = BVHAccel::getIntersection(node->left, ray);//左叶子节点
    Intersection hit2 = BVHAccel::getIntersection(node->right, ray);//右叶子节点
