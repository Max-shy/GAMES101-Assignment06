    ...
    const std::array<int, 3>& dirIsNeg) const
{
    ...
    //get the intersection of the ray with three pairs of parallel axes
    ...
    float Tenter = std::max(tx_min,std::max(ty_min,tz_min));
    float Texit = std::min(tx_max,std::min(ty_max,tz_max));
